...
export class BookingPaymentComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    this.bookingCartSub = this.cartservice.bookingCartChanged.subscribe((cart) => {
      if (!localStorage.getItem('bookingCart') && !cart) {
        ...
        this.displayCart = this.cartservice.displayBookingCartGetter(this.bookingCart);
      } else if (!localStorage.getItem('bookingCart') && cart) {
  ...
  onSubmit(f: NgForm) {
    ...
    this.userS.createBooking(this.bookingCart).subscribe(() => {
